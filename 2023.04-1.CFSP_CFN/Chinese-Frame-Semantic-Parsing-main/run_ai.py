...
from dataset import FrameAIDataset #, DataCollatorForFrameAI
import logging
import math
...
def Predict(args, accelerator, model, eval_dataset, eval_dataloader):
    label_names = ['O', 'B', 'I']
    # Evaluation
    ...
    for step, batch in enumerate(eval_dataloader):
        with torch.no_grad():
            ...
            # all_spans = []
            for tid, word_id, pred in zip(task_id, word_ids, true_predictions):
                tid = tid[0]
                word_id = word_id[1:len(pred)+1]
                tags = ['O'] * (word_id[-1] + 1) 
                ...
                for i, tag in enumerate(tags):
                    if tag == 'B' and span_start == -1:
                        span_start = i
                    elif tag == 'B' and span_start != -1:
                        spans.append((tid, span_start, i-1))
                        span_start = i
                    elif tag == 'O' and span_start != -1:
                        spans.append((tid, span_start, i-1))
                        span_start = -1
                    elif i == len(tags) - 1 and span_start != -1:
                        spans.append((tid, span_start, i))
                        span_start = -1
                # print(spans)
                # exit(0)
    ...
    with open('./ccl-cfn/result/task2_test.json', 'w') as f:
...
def train(args, accelerator, model, train_dataset, train_dataloader, optimizer, lr_scheduler, eval_dataset, eval_dataloader, tokenizer):
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        #     total_loss = 0
        # if epoch == args.num_train_epochs1:
        #     train_dataset = FrameSRLDataset(args.train_file2, tokenizer)
        ...
        precision, recall, F1 = Evaluate(args, accelerator, model, eval_dataset, eval_dataloader, metric)
...
def main():
    ...
    with accelerator.main_process_first():
        train_dataset = FrameAIDataset(data_files['train'], tokenizer)
    ...
    if "validation" not in data_files:
        raise ValueError("--do_train requires a train dataset")
    with accelerator.main_process_first():
        eval_dataset = FrameAIDataset(data_files['validation'], tokenizer)
        if args.max_eval_samples is not None:
            eval_dataset = eval_dataset.subset(range(args.max_eval_samples))    

    if args.do_predict:
        test_dataset = FrameAIDataset(data_files['test'], tokenizer)
    ...
    data_collator = DataCollatorForTokenClassification(tokenizer=tokenizer, pad_to_multiple_of=(8 if accelerator.use_fp16 else None))
    ...
    if args.do_predict:
        ...
        Predict(args, accelerator, model, test_dataset, test_dataloader)
