...
#############pytorch net##################
# This file contains the implementation of three neural network models: LSTM, RNN, and CNNNet.
# The LSTM and RNN models are recurrent neural networks, while the CNNNet model is a convolutional neural network.
# Each model is defined as a class that inherits from the nn.Module class of the PyTorch library.
# The LSTM and RNN classes have a constructor that initializes the layers of the network and a forward method that defines the forward pass of the network.
# The CNNNet class also has a constructor that initializes the layers of the network and a forward method that defines the forward pass of the network.
...
def reg_stats(y_true, y_pred, scaler=None):
    """
    Computes the R2 score and mean absolute error (MAE) between the true and predicted values.

    Args:
        y_true (numpy.ndarray): The true values.
        y_pred (numpy.ndarray): The predicted values.
        scaler (sklearn.preprocessing.StandardScaler, optional): The scaler used to scale the data. Defaults to None.

    Returns:
        tuple: A tuple containing the R2 score and MAE.
...
def train(df, target, hparam):
    ...
    for train_index, test_index in kf.split(X):
        ...
        """
        通过csv文件中给出的linker，使用rdkit库得到fingerprint,
        将linker特征转化为数字向量特征
        """
        if use_rdkit:
        ...
        for epoch in range(epochs):
            ...
            net.train()
            # y_pred_train = net(torch.tensor(x_train).float())  # CNNNet
            y_pred_train = net(
