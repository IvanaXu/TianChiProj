...
class LSTM(nn.Module):
    def __init__(self, input, output, midlayer1, midlayer2):
        ...
        """
        self.lstm = nn.Sequential(nn.LSTM(input, midlayer1, 2, batch_first=True))
        self.full = nn.Sequential(
            nn.Linear(in_features=midlayer1, out_features=midlayer2, bias=True),
            nn.LeakyReLU(negative_slope=0.05),
            nn.Dropout(0.2),
...
class RNN(nn.Module):
    def __init__(self, input, output, midlayer1, midlayer2):
        ...
        """
        self.rnn = nn.Sequential(nn.RNN(input, midlayer1, batch_first=True))
        self.full = nn.Sequential(
            nn.Linear(in_features=midlayer1, out_features=midlayer2, bias=True),
            nn.LeakyReLU(negative_slope=0.05),
            nn.Dropout(0.2),
...
def reg_stats(y_true, y_pred, scaler=None):
    """
    Calculates the R2 score and mean absolute error (MAE) between the true and predicted values.
...
def train(df, target):
    ...
    for train_index, test_index in kf.split(X):
        ...
        # Scaling the predicted and true values back to their original scale
